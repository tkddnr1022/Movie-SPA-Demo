                    ...
                    <div :class="{ 'card': true, 'hidden': !isLoginVisible }" id="login">
                        <form @submit.prevent="handleLogin">
                            ...
                            <div class="input" :class="{ 'active': isEmailFocused || email }">
                                <input id="email" type="email" v-model="email" @focus="focusInput('email')"
                                    @blur="blurInput('email')" />
                                <label for="email">Username or Email</label>
                            </div>
                            <div class="input" :class="{ 'active': isPasswordFocused || password }">
                                <input id="password" type="password" v-model="password" @focus="focusInput('password')"
                        ...
                        <a href="javascript:void(0)" class="account-check" @click="toggleCard">Already have an account?
                            <b>Sign in</b></a>
                    </div>
                    ...
                    <div :class="{ 'card': true, 'hidden': isLoginVisible }" id="register">
                        <form @submit.prevent="handleRegister">
                            ...
                            <div class="input" :class="{ 'active': isRegisterEmailFocused || registerEmail }">
                                <input id="register-email" type="email" v-model="registerEmail"
                                    @focus="focusInput('registerEmail')" @blur="blurInput('registerEmail')" />
                                <label for="register-email">Email</label>
                            </div>
                            <div class="input" :class="{ 'active': isRegisterPasswordFocused || registerPassword }">
                                <input id="register-password" type="password" v-model="registerPassword"
                                    @focus="focusInput('registerPassword')" @blur="blurInput('registerPassword')" />
                            ...
                            <div class="input" :class="{ 'active': isConfirmPasswordFocused || confirmPassword }">
                                <input id="confirm-password" type="password" v-model="confirmPassword"
                                    @focus="focusInput('confirmPassword')" @blur="blurInput('confirmPassword')" />
                        ...
                        <a href="javascript:void(0)" id="gotologin" class="account-check" @click="toggleCard">Don't have
                            an account? <b>Sign up</b></a>
                    </div>
                </div>
            </div>
        </div>
...
export default {
    ...
    setup() {
        ...
        const handleLogin = () => {
            ...
            // AuthService 활용하여 API 호출 및 로그인 처리
        };

        const handleRegister = () => {
            console.log('회원가입 시도:', { email: registerEmail.value, password: registerPassword.value, confirmPassword: confirmPassword.value });
            // AuthService 활용하여 API 호출 및 회원가입 처리
        };
